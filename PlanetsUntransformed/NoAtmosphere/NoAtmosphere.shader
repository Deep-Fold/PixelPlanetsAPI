...
uniform float light_border_1 : hint_range(0.0, 2.0) = 0.4;
uniform float light_border_2 : hint_range(0.0, 2.0) = 0.6;
...
void fragment() {
	...
	if (d_light > light_border_1) {
		col = color2;
		if (d_light < light_border_1 + dither_border && (should_pixelize && dith || !should_dither)) {
			col = color1;
		}
	}
	if (d_light > light_border_2) {
		col = color3;
		if (d_light < light_border_2 + dither_border && (should_pixelize && dith || !should_dither)) {
			col = color2;
		}
	}
